...
//	sendcom.c
//
//	USBåoóRÇ≈ÉAÉCÉeÉBÉvÉâÉìÉ^Å[Ç…ÉRÉ}ÉìÉhÇëóêMÇµÇ‹Ç∑ÅBÉAÉCÉeÉBÉvÉâÉìÉ^Å[Ç©ÇÁÇÃâûìöÇéÛÇØéÊÇËÇ‹Ç∑ÅB
//
...
//	if( libusb_interrupt_transfer
	if( libusb_bulk_transfer
       (handle, EP_OUT, command, INTR_LENGTH, &transferred, 1000)!= 0)
    {
	...
	// read data
	//if( libusb_interrupt_transfer(handle, EP_IN, rcommand, INTR_LENGTH, &
...
char* Process(char c,int n)
{
	...
	switch(c){
		case 'A':
			...
			//ÇSÇOÇOÇçVéûÇ™ÇOìxÇ≈ÇPÇXÅDÇTÇçVÅ^ÅéÅ@Ç…Ç»ÇËÇ‹Ç∑ÅB
...
void command_list()
{
	...
	printf("^   Set current time to real time clock.\n");
...
//	printf("\n");
	printf("e read tmp/illum recorde\n");
		...
		if(strcmp(argv[i],"-e") == 0 ){
			...
			if(idata[0]=='%'){
				...
				if( idata[0] == '*' ){
					...
					// single command 
					printf("Command: %s\n",idata);
					if( strcmp(idata,"^")==0 ) Process( 'G',n);  // Clock setÅ@Å@type '^'
#ifdef OLDVERSION
			...
			// reset planter
			// retryÇ™ãNÇ´ÇƒÇ¢ÇΩÇÁÅAÉäÉZÉbÉgÇµÇƒÇ®Ç≠ÅB
			if(resetcom){
